...
<script type="text/javascript">
    ...
    function DeleteReturnZendeskTicketNo() {
        var currentTicketNo = $("[id$=lblCurrentZendeskTicketNoReturn]").text();
        $.ajax({
            type: "POST",
    ...
    function UpdateReturnZendeskTicket() {
        var currentTicketNo = $("[id$=lblCurrentZendeskTicketNoReturn]").text();
        var updateTicketNo = $("[id$=txtUpdateZendeskTicketNoReturn]").val();
    ...
    function CloseReturnDialog() {
        $('#zendeskOptionsReturns').dialog('close');
        return false;
    };

    function ZendeskOptionsReturns(ticketNo) {
        $('#zendeskOptionsReturns').dialog({
            title: 'Please select an option.',
            show: true,
            modal: true
        });

        $("[id$=lblCurrentZendeskTicketNoReturn]").text(ticketNo);

        return false;
    };
</script>

<div id="zendeskOptionsReturns" style="display: none">
    <asp:Label ID="lblCurrentZendeskTicketNoReturn" runat="server" />
    <br />
    <asp:TextBox ID="txtUpdateZendeskTicketNoReturn" runat="server" />
    <br />
    <asp:Button ID="btnUpdateZendeskTicketNoReturn" Text="Update Zendesk Ticket" runat="server" OnClientClick="UpdateReturnZendeskTicket()" />
    <asp:Button ID="btnDeleteZendeskTicketNoReturn" Text="Delete Zendesk Ticket" runat="server" OnClientClick="DeleteReturnZendeskTicketNo()" />
    <asp:Button ID="btnCancelZendeskOptionsReturn" Text="Cancel" runat="server" OnClientClick="CloseReturnDialog()" />
</div>

<asp:Table ID="SingleReturnOrderZendeskTicketsTable" runat="server" Height="100%" Width="75%" BorderColor="Black" BorderStyle="Solid" BorderWidth="2px">
